...
CROSS_COMPILE = armv6k-openwrt-linux-
endif
endif
ifeq ($(ARCH),i386)
...
#########################################################################
## Cavium Networks cnsxxxx Platform Baseboard
#########################################################################
...
cavium_cns3000be8_config : unconfig
	@if [ "$(findstring _cns3000be8_, $@)" ] ; then \
		...
		$(XECHO) "... configured for big-endian (BE8) system"; \
	else \
		sed -r 's/ENDIANNESS/little/g; s/#.*//' board/cavium/cns3000/u-boot.lds.template \
			> board/cavium/cns3000/u-boot.lds ; \
		$(XECHO) "... configured for little-endian system"; \
	fi;
	@$(MKCONFIG) $(@:_config=) arm arm11mpcore cns3000 cavium
